...
description: Find an ebook or audiobook by title, author, or publisher and identify independent bookshops that carry it. Use when someone asks where to buy or find a digital book, requests availability in a language, wants free-book results, or wants current store offers without an automated checkout.
---
...
4. If a store includes a price, repeat the amount, currency, and freshness timestamp when useful. A missing price means unknown, not free.
5. Present the returned bookshop product URL or `checkout_url` for the user to open themselves.

## Safety and accuracy

- Never fetch or open `checkout_url`; doing so can create a cart on the bookshop.
- Never estimate missing prices, convert currencies without being asked, or label a shop cheapest.
